...
AnicvacDagax = require('./AnicvacDagax');
    ...
    for (var y = 0; y < matrix.length; y++) {
        for (var x = 0; x < matrix[y].length; x++) {
            if (matrix[y][x] == 1) {
                ...
                matrix[y][y] == 4
                AnicvacDagaxArr.push(new AnicvacDagax(x, y, 4))
            }
...
function game() {
    ...
    for (var i in AnicvacDagaxArr) {
        AnicvacDagaxArr[i].eat();
    }
    ...
    dataSend = {
        ...
        anicvacdagaxCount: AnicvacDagaxArr.length,
...
io.on('connection', function (socket) {
    ...
    socket.on("add AnicvacDagax", addAnicvacDagax);
...
setInterval(function () {
    ...
    statistics.AnicvacDagax = AnicvacDagaxArr.length;
